...
const Format = ({ref}:IDropdownProps) => {
    ...
    const {updateFormat, toggleDropdown, setChangeFormat} = useActions()
            ...
            <div className="wrap-select-lg-s" style={{maxWidth:'100px'}} ref={ref}>
                ...
                <ul className="list-lg">
                    {DropdownItems.Format.map((item) => (
                        <li key={item.name} className="list-lg-item" style={{maxWidth:'100px'}} onClick={() => {updateFormat(item.value);toggleDropdown(0); setChangeFormat(true)}}>
